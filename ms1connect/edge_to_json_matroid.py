...
def jsonHelperPartitionMatroid(dic, limit, name):
    blockArray = []
    for key in dic:
        curBlock = {"block": dic[key], "limit": limit}
        blockArray.append(curBlock)
    singleMatroid = {"partition-matroid": {"name": name, "blocks": blockArray}}
    return singleMatroid


def makeJson(leftDic, rightDic, limit=1, name="bipartite graph"):
    leftMatroid = jsonHelperPartitionMatroid(
        leftDic, limit, "edges incident to left nodes,V"
    )
    rightMatroid = jsonHelperPartitionMatroid(
        rightDic, limit, "edges incident to right nodes,U"
    )
