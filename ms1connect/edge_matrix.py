...
def fillInMatrix(
    ...
    countTermSum = np.float32(0.0)
    intensTermSum = np.float32(0.0)
    ...
    for i in range(0, nRow):
        for j in range(i, nRow):
            ...
            if i == j:  # edge1 == edge2
                ...
                # intensity term
                intensTerm = (
                    leftFile[edge1Left, intensCol] * rightFile[edge1Right, intensCol]
                )
                ...
                pticTerm = math.exp(
                    -alpha1
                    * abs(leftFile[edge1Left, pticCol] - rightFile[edge1Right, pticCol])
                )
                ...
                # edge shift term
                shiftTerm = math.exp(
                    -alpha2 * abs(edge1[ticDiffIndex] - edge2[ticDiffIndex])
                )

                startTerm = math.exp(-alpha3 * abs((edge1LeftpTIC - edge2LeftpTIC)))
    ...
    postTermNorm = (
        (lambda1 * countTermSum)
        + (lambda2 * intensTermSum)
        + (lambda3 * pticTermSum)
        + (lambda4 * edgeSimTermSum)
    )
...
def createEdgeSimMatrix(
    ...
    newFileName = Path(outputFolderName) / Path(
        edgeFileName_basename + "___pairwise.npz"
    )
    ...
    if nRow != 0:
        ...
        leftFileName, rightFileName = getLeftRightFile(
            edgeFileName_basename, peakFolderName
        )
        ...
        rowList, colList, valList, diagRow, diagScore, postNormVal = fillInMatrix(
            edgeFile,
            leftFile,
            rightFile,
            nRow,
    ...
    sparseMat = scipy.sparse.csr_matrix(
        (valList_np, (rowList_np, colList_np)), shape=(nRow, nRow)
    )
