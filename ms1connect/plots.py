    ...
    with open(scoreFile, "r") as file1:
        for line1 in file1:
            if line1.startswith("filename___"):
                ...
                if leftFile not in ms1FileList or rightFile not in ms1FileList:
                    leftFileIndex = -1
                    rightFileIndex = -1
                else:
                ...
                if leftFileIndex == -1 and rightFileIndex == -1:
                    continue
...
def assertDiagonal(curRunMatrix):
    ...
    for i in range(0, nRow):
        assert curRunMatrix[i][i] != 0, (
            "Metadata file contains file that was not present in score file. Remove from metadata file. %s line"
            % (i)
        )
    return
...
###############################################################################
def createRunSimMatrix(
    ms1PeakFolderName, scoreFileName, metadataFileName, edgeCountFileName, output_folder
):
    """
    Main driver script
    """
    ...
    np.savetxt(
        output_folder + "/output_score_matrix.txt", runMatrix, delimiter="\t", fmt="%f"
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="From baseline metric output "
