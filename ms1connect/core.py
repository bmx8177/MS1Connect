...
def ms1Connect(
    ...
    create_edge.create_edges(
        ms1_folder,
        edge_folder,
        str(Path(__file__).resolve().parent) + "/bin/createEdge",
        mz_tol,
        tic_tol,
    )
    ...
    with (
        open("pairwise-edge.log.txt", "w") as file1,
        ...
        for f in b:
            fileName, nRow, rowListSize, postNormVal = (
                pairwise_edge_matrix.createEdgeSimMatrix(
                    str(f),
                    ms1_folder,
                    edge_sim_folder,
                    lambda1,
            ...
            cmd = (
                "singularity exec --bind ./:/input/ --bind "
                + "./:/output/ "
    ...
    plots.createRunSimMatrix(
        ms1_folder,
        "coopraize.log.txt",
        metadata_file,
        "pairwise-edge.log.txt",
        output_folder,
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Runs MS1Connect on a set of \
mzML files."
    )
    ...
    parser.add_argument(
        "matroid",
        help="Folder containing matroid files.\
						Folder will be created if it does not exist.",
    )
    parser.add_argument(
        "edgeSimMatrix",
        help="Folder containing sparse edge\
    ...
    parser.add_argument(
        "--topN",
        help="Keep top N most intense MS1 features.\
	Default=4000",
        type=int,
        default=4000,
    )
    parser.add_argument(
        "--mzTol",
        help="m/z tolerance in ppm to create an edge.\
 	Default=4",
        type=float,
        default=4,
    )
    parser.add_argument(
        "--ticTol",
        help="TIC tolerance to create an edge.\
	Default=1",
        type=float,
        default=1.0,
    )
    parser.add_argument(
        "--lambda1", help="lambda 1 hyperparameter. Default=0.0", default=0, type=float
    )
    parser.add_argument(
        "--lambda2",
        help="lambda 2 hyperparameter. Default=0.1",
        default=0.1,
        type=float,
    )
    parser.add_argument(
        "--lambda3",
        help="lambda 3 hyperparameter. Default=0.0",
        default=0.0,
        type=float,
    )
    parser.add_argument(
        "--lambda4",
        help="lambda 4 hyperparameter. Default=0.9",
        default=0.9,
        type=float,
    )
    parser.add_argument(
        "--alpha", help="alpha hyperparameter. Default=0.0", default=0.0, type=float
    )
    parser.add_argument(
        "--beta", help="beta hypereparameter. Default=1e-5", default=0.00001, type=float
    )
    parser.add_argument(
        "--gamma", help="gamma hyperparameter. Default=1.0", default=1.0, type=float
    )
