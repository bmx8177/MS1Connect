...
def create_edges(inputFolderName, outputFolderName, binaryPath, mz_tol, tic_tol):
    ...
    for i in range(0, len(fileList)):
        for j in range(i, len(fileList)):
            ...
            outFile = (
                outputFolderName
                + "/"
                + cleanFileName(leftFile)
                + "___"
                + cleanFileName(rightFile)
                + "___score.txt"
            )
            ...
            if Path(outFile).is_file():
                continue
            subprocess.call(
                [binaryPath, leftFile, rightFile, outFile, str(mz_tol), str(tic_tol)]
            )
