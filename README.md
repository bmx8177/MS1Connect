...
Before running MS1Connect, we highly suggest you create a new environment (such as conda).

The inputs to MS1Connect are a set of MS1 features files. MS1Connect can generate these files for you using pyOpenMS by providing a folder of mzML files. If you prefer to use your own MS1 feature detection method, you can instead provide a folder of MS1 feature files that contains the following columns: m/z, intensity, retention time, normalized retention time (proportion of TIC), and charge.

Running MS1Connect requires executing the `ms1connect.py` script. You can see how to run this script by executing the following command:
...
The manuscript describing MS1Connect can be found `here <https://academic.oup.com/bioinformatics/article/39/2/btad058/7005198>`_.
